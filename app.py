...
if uploaded_file is not None:
    ...
    with col2:
        ...
        # --- TAB 1: Auto Summary ---
        with tab_summary:
            st.info("ℹ️ The AI will analyze audio patterns to keep the top **30% most important moments**.")
        ...
        with tab_search:
            st.info("ℹ️ Find every exact moment a specific word is spoken.")
            ...
            if st.button("🔎 Find & Clip", key="btn_search"):
                if not search_term:
                    ...
                    with st.spinner(f"🕵️ Scanning for '{search_term}'..."):
                        segments = backend.transcribe_video(temp_path)
